...
def find_connected_components_bfs(graph):
    visited = set()        # To keep track of all visited vertices
    components = []        # List to store all connected components

    # Build an undirected adjacency list from the directed graph
    adjacency = {v: set() for v in graph.parse_vertices()}  # Initialize empty neighbor sets
    for v in graph.parse_vertices():
        # Add all outbound edges (v → neighbor)
        for _, neighbor in graph.get_outbound_edges(v):
            adjacency[v].add(neighbor)
            adjacency[neighbor].add(v)  # Add reverse for undirected behavior

        # Add all inbound edges (neighbor → v)
    ...
    for vertex in adjacency:
        if vertex not in visited:
            component = set()         # Store nodes in this connected component
            queue = [vertex]          # Initialize queue with the starting node
            visited.add(vertex)       # Mark it as visited

            # Standard BFS loop
            while queue:
                current = queue.pop(0)        # Dequeue the next node
                component.add(current)        # Add it to the current component

                # Visit all unvisited neighbors
                for neighbor in adjacency[current]:
                    if neighbor not in visited:
                        visited.add(neighbor) # Mark neighbor as visited
                        queue.append(neighbor) # Enqueue it for further exploration

            components.append(component)      # Save the found component

    return components  # Return the list of all connected components
