...
void testRemoveBetween() {
	...
	SortedIndexedList list([](TComp a, TComp b) { return a <= b; });
	...
	assert(list.getElement(4) == 9);

	// Remove elements between positions 1 and 3 = removes 3, 5, 7
	list.removeBetween(1, 3);

	assert(list.size() == 2);
	assert(list.getElement(0) == 1);
	assert(list.getElement(1) == 9);
